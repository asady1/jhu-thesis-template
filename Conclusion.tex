\chapter{Conclusion}\label{Sec:Ana}
...
The impact of the resolved (green), semi-boosted (aqua), and boosted (royal blue), where no shared events have been rejected for any of the selections, can be seen in Fig.~\ref{fig:limcompare}, for bulk graviton and radion. Semi-boosted does best between roughly 1 and 1.4 TeV.
\begin{figure}
\centering
...
\section{Results Non resonant}
\section{Results\label{sec:Results}}
%Limits are reported in units of fb for v0 benchmark samples. This will be updated when v1 benchmark samples are available. 
Systematics on the combined channels can be summarized as follows:

\begin{table}[h]
  \topcaption{Summary of systematic uncertainties in the signal and background yields.}
  \label{tab:Syst}
  \begin{center}
    \begin{tabular}{lc}
      \hline\hline
...
The observed and expected limits are in Tab.~\ref{tab:2p1lim} for semi-boosted (rejecting boosted events) and boosted. The combined limit of the two channels is in Tab.~\ref{tab:2p1limcomb}. Observed limits are within 95\% expected limits. % and the 68\% and 95\% expected limits for the combined channel are in Tab.~\ref{tab:2p1limcomb}.

\begin{table}[h]
\topcaption{Expected and observed limits for semi-boosted (rejecting boosted events) and boosted separately for v1 benchmark model, SM.}\label{tab:2p1lim}
...
\begin{table}[h]
\topcaption{Observed and expected for the combined semi-boosted + boosted limit.}\label{tab:2p1limcomb}
...
\begin{table}[h]
\topcaption{Observed and expected combined boosted + semi-boosted limits including rejection of the resolved events (HIG-17-017 selection).}\label{tab:limNoRes}
\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
