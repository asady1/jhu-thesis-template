...
% Do not edit by hand!
\RCS$Revision: 435774 $
\RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/notes/AN-17-182/trunk/AN-17-182.tex $
\RCS$Id: AN-17-182.tex 435774 2017-11-22 16:35:17Z asady $
%%%%%%%%%%%%% local definitions %%%%%%%%%%%%%%%%%%%%%
...
\newcommand{\mjjs}{\ensuremath{M_{\rm jjj}^{\rm red}}\xspace}
\newcommand{\Mjjs}{\ensuremath{M_{\rm jjj}^{\rm red}}\xspace}
\newcommand{\mjj}{\ensuremath{M_{\rm jjj}}\xspace}
\newcommand{\Mjj}{\ensuremath{M_{\rm jjj}}\xspace}
...
%%%%%%%%%%%%%%%  Title page %%%%%%%%%%%%%%%%%%%%%%%%
\cmsNoteHeader{AN-17-182} % This is over-written in the CMS environment: useful as preprint no. for export versions
% >> Title: please make sure that the non-TeX equivalent is in PDFTitle below
\title{Search for resonances decaying to a pair of Higgs bosons in the four b quark final state in proton-proton collisions at sqrt(s) = 13 TeV}

\author[all]{Ching-Wei~Chen, Alice~Cocoros, Leonardo~Giannini, Maxime~Gouzevitch, Michael~Krohn, Devdatta~Majumder, Petar~Maksimovic, Marc~Osherson, Andrea~Rizzi, Caterina~Vernieri, Shin-Shan~Eiko~Yu}
% >> Authors
...
pdftitle={Search for resonances decaying to a pair of Higgs bosons in the four b quark final state in proton-proton collisions at sqrt(s) = 13 TeV},%
pdfsubject={CMS},%
...
\include{APP_AK8AK4choice}
\include{APP_deta}
\include{APP_trigger}
\include{APP_ptoptimization}
\include{APP_massoptimization}
\include{APP_tau21optimization}
\include{APP_doubleboptimization}
\include{APP_deepCSV}
\include{APP_AK4variables}
\include{APP_ttbar}
\include{APP_boosted}
\include{APP_redm}
\include{APP_background}
\include{APP_overlap}
...
%If you wish to export your paper (for local work or for security), you can produce a t%arball with all the necessary files with
%\begin{verbatim}
%> tdr --style=note --export b mynote.
%\end{verbatim}
%This will function on  Unix or Windows systems which have recent copies of \LaTeX\ (in%cluding \AmS-\LaTeX) and \texttt{perl} installed. We currently use the \texttt{sectsty%, subfig,
...
%When Greek or symbol characters are used in the title, author, keywords or section hea%ds, please use the \verb|\texorpdfstring|
%command to provide alternate versions. Acrobat cannot deal with \TeX\ characters and w%ill ignore many of them for your PDF bookmark. See the following two subsections and c%heck the corresponding bookmarks. (You may notice that this will produce four instances of ``Package hyperref Warning: Token not allowed in a PDFDocEncoded string" in the output log.)
...
%Place the captions above the object for tables and use \texttt{topcaption}, below for %figures using \texttt{caption}. To force a full width figure or table in the two-colum%n mode of most journal reprint formats, use \verb|\textwidth| as the unit along with the starred versions of the commands:
...
%\verb|\oddsidemargin| & \the\oddsidemargin & \verb|\evensidemargin| & \the\evensidemar%gin \\
...
%sizes to which the figures will be scaled. Specifying both width and height forces bot%h
%dimensions to be changed and causes a distortion of the figure, however, so
%only use one of the two. Don't try to use scaling to correct a bad original aspect rat%io. If neither width nor height is given, the size is
...
%translators on lxplus are \texttt{epstopdf} and \texttt{ps2pdf -dEPSCrop}. Native Post%Script can not be included.

%The \texttt{subfig} package is included and can be used for PASs and ANs (but not pape%rs) to generate (a), (b), \etc labels under the subfigures through the use of the \texttt{subfloat} command. We have aliased \texttt{subfigure} to \texttt{subfloat} to avoid breaking older documents which may have depended on the \texttt{subfigure} package, but the spacing will not necessarily be the same. You may need to add line breaks by hand.
...
% %   \subfloat[]{\includegraphics[width=0.2\textwidth]{CMS-bw-logo}}\subfloat[]{\inclu%degraphics[width=0.2\textwidth]{CMScol}}
...
%be over-ridden using the \verb|\renewcommand| command. If you don't want to over-ride the default version of a command but provide it for use outside the normal tdr system, pl%ease use the \verb|\providecommand| command.
